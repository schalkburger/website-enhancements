...
(function ($, Mediabunny) {
  ...
  const PARENT_CHILD_MAPPING = {
    ...
    "FORCE_RESOURCE_VIA_MEDIA": ["FALLBACK_TO_BLOB_FETCH_IF_MEDIA_API_THROTTLED", "NEW_TAB_ALWAYS_FORCE_MEDIA_IN_POST", "PREFER_DASH_MANIFEST"],
  };
  ...
  var timer = setInterval(function () {
    ...
    if (
      ...
      location.pathname.match(/^\/(explore(\/.*)?|challenge\/?.*|direct\/?.*|qr\/?|accounts\/.*|emails\/.*|language\/?.*?|your_activity\/?.*|settings\/help(\/.*)?$)$/gi) ||
    ...
    if (state.currentURL != location.href || !state.firstStarted || !state.pageLoaded) {
      ...
      if (location.pathname.startsWith("/p/") || location.pathname.match(/^\/(.*?)\/(p|reel)\//gi) || location.pathname.startsWith("/reel/")) {
      ...
      if (location.pathname === "/") {
        ...
        let hasReferrer = state.GL_referrer?.match(/^\/(stories|highlights)\//gi) != null;
      ...
      if (
        ...
        location.pathname.match(/^(\/)([0-9A-Za-z\.\-_]+)\/?(tagged|reels|saved)?\/?$/gi) &&
  ...
  async function onHighlightsStory(isDownload, isPreview) {
    ...
    if (isDownload) {
      ...
      let highlightId = location.href.replace(/\/$/gi, "").split("/").at(-1);
      let nowIndex =
        $("body > div section._ac0a header._ac0k > ._ac3r ._ac3n ._ac3p[style]").length ||
      ...
      .each(function (index) {
        ...
        if (!$(this).attr("data-snig") && !$(this).hasClass("x1iyjqo2") && !$(this).children("article")?.hasClass("x1iyjqo2") && $(this).parents("div#scrollview").length === 0) {
          ...
          state.GL_registerEventList.push({
            element: this,
            trigger: [".IG_THUMBNAIL_MAIN", ".IG_NEWTAB_MAIN", ".IG_DW_ALL_MAIN", ".IG_DW_MAIN", ".IG_IMAGE_VIEWER"],
          });
          ...
          $(this).on("click", ".IG_THUMBNAIL_MAIN", function () {
            ...
            state.GL_postPath =
              ...
              $mainElement.find('a[href^="/p/"]').first().attr("href").split("/").at(2) ||
              $(this).parent().parent().parent().children("div:last-child").children("div").children("div:last-child").find('a[href^="/p/"]').last().attr("href").split("/").at(2);
            ...
            createMediaListDOM(state.GL_postPath, ".IG_POPUP_DIG .IG_POPUP_DIG_MAIN .IG_POPUP_DIG_BODY", "").then(() => {
              let checkBlob = setInterval(() => {
                if ($(".IG_POPUP_DIG .IG_POPUP_DIG_MAIN .IG_POPUP_DIG_BODY a").length > 0) {
                  ...
                  var $videoThumbnail = $('.IG_POPUP_DIG .IG_POPUP_DIG_BODY a[data-globalindex="' + (index + 1) + '"]')
                    ?.parent()
                    .find(".videoThumbnail")
                    ?.first();
              ...
              var publish_time = new Date(
                $(this)
                  ...
                  .filter(function () {
                    ...
                    return href?.startsWith("/p/") || href?.match(/\/([\w.\-_]+)\/p\//gi) != null;
                  })
                  .first()
                  .attr("datetime"),
              ).getTime();
                  ...
                  $(this)
                    ...
                    .each(function () {
                      ...
                      if (element_images && imgLink) {
                        $(".IG_POPUP_DIG .IG_POPUP_DIG_MAIN .IG_POPUP_DIG_BODY").append(
          ...
          var username =
            $(this).find("header > div:last-child > div:first-child span a").first().text() ||
            $(this)
              .find('a[href^="/"]')
              .filter(function () {
  ...
  async function onStoryAll() {
    ...
    let timestamp = Math.floor(date / 1000);
    let username =
      $("body > div section._ac0a header._ac0k ._ac0l a + div a").first().text() ||
      location.pathname
        .split("/")
        .filter((s) => s.length > 0)
        .at(1);
  ...
  function getStoryProgress(username) {
    let $header = $('body > div section:visible a[href^="/' + username + '"] span')
      .filter(function () {
  ...
  function parseDashManifest(mpdXml) {
    try {
      ...
      const bestVideo = candidates.filter(isVideo).sort((a, b) => b.height - a.height || b.bandwidth - a.bandwidth || b.width - a.width)[0] || null;
  ...
  async function downloadDashStreams(videoUrl, audioUrl, username, sourceType, timestamp, shortcode) {
    ...
    try {
      ...
      const [vBuf, aBuf] = await Promise.all([fetchArrayBuffer(videoUrl), fetchArrayBuffer(audioUrl)]);
   ...
   */
  async function tryHandleDashFromMediaItem({ mediaItem, username, sourceType, timestamp, shortcode, isPreview, index }) {
    try {
  ...
  async function changeExifData(blob, metadata) {
    ...
    const exifIfd = concat(u16le(exifIfdCount), makeIFDEntry(0x9003, 2, dateBytes.length, dateOffset), u32le(0));
    ...
    let exifChunk = concat(enc("EXIF"), u32le(exifPrefix.length + tiffBody.length), exifPrefix, tiffBody);
      ...
      if (USER_SETTING.FORCE_RESOURCE_VIA_MEDIA) {
        ...
        if (result.status === "ok") {
          ...
          if (result.items[0].video_versions) {
            ...
            if (result.items[0].original_width !== resourceWidth && resourceWidth !== -1) {
              // alert();
            }
          }
  ...
  function IG_createDM(hasHidden, hasCheckbox) {
    ...
    if (hasCheckbox) {
      ...
      $(".IG_POPUP_DIG .IG_POPUP_DIG_MAIN .IG_POPUP_DIG_TITLE > div#button_group").append(
  ...
  function showHotkeySetting() {
                ...
                <label class="globalSettings hotkey-setting-item" data-hotkey="${name}" style="position: relative;display: flex; align-items: center; padding-right: 5px;">
                    ...
                    <div class="hotkey-select-wrapper" style="display: flex; align-items: center; gap: 8px; justify-content: flex-end; flex: 1;">
                        <select class="hotkey-preset" data-storage="${storageKey}" data-state="${stateKey}" data-default="${defaultKeyCode}" style="padding: 4px 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 13px;">
                            ${hotkeyOptions
                              .filter((o) => o.value != defaultKeyCode.toString())
                              .map((o) => `<option value="${o.value}" ${o.value == currentKeyCode ? "selected" : ""}>${o.label}</option>`)
                              .join("")}
    ...
    hotkeyConfigs.forEach((config) => {
      $body.find(".hotkey-settings-container").append(createHotkeySetting(config.name, config.key, config.stateKey, config.storageKey, config.defaultKeyCode));
    });
  }
  ...
  function showSetting() {
    ...
    for (const o in locale_manifest) {
      $("#langSelect").append(`<option value="${o}" ${state.lang === o ? "selected" : ""}>${locale_manifest[o]}</option>`);
    }
  ...
  $(function () {
    ...
    $(window).on("keydown", function (e) {
      ...
      if (e.altKey && e.which == downloadStoryKeyCode) {
        if (location.href.match(/^(https:\/\/www\.instagram\.com\/stories\/)/gi) && $(".IG_DWSTORY").length > 0) {
          $(".IG_DWSTORY")?.trigger("click");
        }
        if (location.href.match(/^(https:\/\/www\.instagram\.com\/stories\/highlights\/)/gi) && $(".IG_DWHISTORY").length > 0) {
    ...
    $("body").on("click", ".IG_POPUP_DIG_BODY .videoThumbnail", function () {
      ...
      saveFiles($(this).parent().children("a").find("img").first().attr("src"), {
          ...
          mutation.addedNodes.forEach((node) => {
            ...
            if ($videos.length > 0) {
              ...
              if (location.pathname.match(/^(\/stories\/)/gi)) {
                const isHighlight = location.pathname.match(/^(\/stories\/highlights\/)/gi) != null;
                  ...
                  if (USER_SETTING.HTML5_VIDEO_CONTROL) {
                    if (!$video.data("controls")) {
                      ...
                      let $element_mute_button = $videoParent
                        .parent()
                        .find('svg > path[d^="M1.5 13.3c-.8 0-1.5.7-1.5 1.5v18.4c0"], svg > path[d^="M16.636 7.028a1.5 1.5"]')
                        .parents('[role="button"]')
                        .first();
